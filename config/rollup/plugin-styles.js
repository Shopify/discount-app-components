    ...
    async transform(source, file) {
      ...
      try {
        sassOutput = sass
          ...
          .css.toString()
          .replace('{{DISCOUNT_APP_COMPONENTS_VERSION}}', packageJSON.version);
      } catch (err) {
        throw new Error(err.formatted);
      }

      const postCssOutput = await styleProcessor
        .process(sassOutput, {from: file})
        .then((result) => ({
          css: result.css,
      ...
      if (mode === 'standalone') {
        return transformStandalone(this, file, postCssOutput);
      }
      if (mode === 'esnext') {
        return transformEsNext(this, file, postCssOutput);
      }
    },
